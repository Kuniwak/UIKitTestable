...
internal typealias SerialDispatcher = (@escaping () -> Void) -> Void


internal func asyncSerialDispatcher(qos: DispatchQoS.QoSClass = .default) -> SerialDispatcher {
...
internal enum DispatchQueueStrategy {
    internal static let `default` = { (callback: @escaping () -> Void) in
        DispatchQueue.global().async(execute: callback)
    }
}


internal func parallel<T1, T2>(
    _ f: @escaping (@escaping (T1) -> Void, @escaping (T2) -> Void) -> Void,
            ...
            serialDispatcher {
                serialDispatcher {
                    ...
                    case .f1Completed(let x1):
                        callback(x1, x2)
                    case .f2Completed:
                        print("WARNING: the 2nd argument of parallel was called more than once via \(debugInfo.file) at \(debugInfo.line).")
                    }
                }
            }
        }
...
internal func parallel<T1, T2>(
    ...
    parallel({ (callback1: @escaping (T1) -> Void, callback2: @escaping (T2) -> Void) in
        f1(callback1)
        f2(callback2)
    }, callback)
}
...
internal func parallel<T1, T2, T3>(
    _ f1: @escaping (@escaping (T1) -> Void) -> Void,
    _ f2: @escaping (@escaping (T2) -> Void) -> Void,
...
internal func parallel<T1, T2, T3, T4>(
    _ f1: @escaping (@escaping (T1) -> Void) -> Void,
    _ f2: @escaping (@escaping (T2) -> Void) -> Void,
    _ f3: @escaping (@escaping (T3) -> Void) -> Void,
